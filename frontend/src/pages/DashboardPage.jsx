...
const formatVND = (n) => {
  ...
  if (num >= 1_000_000_000)
    return (num / 1_000_000_000).toFixed(1).replace(/\.0$/, "") + " tỷ VNĐ";
  if (num >= 1_000_000)
    return (num / 1_000_000).toFixed(1).replace(/\.0$/, "") + " triệu VNĐ";
  if (num >= 1_000) return Math.round(num / 1_000).toString() + " nghìn VNĐ";
  ...
  const kpis = [
    {
      ...
      value: formatVND(today?.revenue || 0),
      icon: "💰",
    ...
    },
    {
      ...
      value: formatVND(month?.revenue || 0),
      icon: "📈",
    ...
    },
    {
      ...
      value: today?.totalOrders
        ? Math.round((today.completedOrders / today.totalOrders) * 100) + "%"
        : "—",
      icon: "🎯",
        ...
        <div className="p-6 space-y-6">
            ...
            {kpis.map((kpi, i) => (
              <div key={i} className={`card border ${kpi.color} p-4`}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs text-coffee-500 leading-tight">
                    {kpi.label}
                  </p>
                </div>
                <p
                  className={`font-display text-2xl font-bold ${kpi.textColor}`}
                >
                  {kpi.value}
                </p>
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  {
                    label: "Nguyên liệu",
                    value: inventory.totalIngredients,
                    icon: "📦",
                  ...
                  },
                  {
                    ...
                    value: formatVND(inventory.totalStockValue),
                    icon: "💎",
                    color: "bg-blue-50 border-blue-200",
                    textColor: "text-blue-700",
                  },
                  {
                    label: "Nhập/Xuất tháng",
                    value: `${inventory.monthImport.count}/${inventory.monthExport.count}`,
                    icon: "🔄",
                    color: "bg-purple-50 border-purple-200",
                    textColor: "text-purple-700",
                  },
              ...
              {inventory.lowStockItems?.length > 0 && (
                <div className="mt-3 bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm text-amber-800">
                  ...
                  {inventory.lowStockItems
                    .map((i) => `${i.tenNL} (${i.tonKho}${i.donVi})`)
                    .join(" · ")}
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="col-span-2 card p-5">
              <h2 className="font-display text-lg text-coffee-900 mb-4">
              ...
              {today?.topItems?.length > 0 ? (
                <>
                  <ResponsiveContainer width="100%" height={160}>
                    <PieChart>
                      <Pie
                        data={today.topItems.slice(0, 5)}
                        dataKey="_sum.soLuong"
                        nameKey="tenMon"
                        cx="50%"
                        cy="50%"
                        outerRadius={70}
                  ...
                  <div className="space-y-1 mt-2">
                    {today.topItems.slice(0, 5).map((item, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        ...
                        />
                        <span className="flex-1 truncate text-coffee-700">
                          {item.tenMon}
                        </span>
                ...
                <p className="text-coffee-400 text-sm text-center py-8">
                  Chưa có dữ liệu
                </p>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="card p-5">
              <h2 className="font-display text-lg text-coffee-900 mb-3">
                Thống kê tháng này
              </h2>
              <div className="space-y-3">
                {[
                  ...
                  { label: "Đơn hủy", value: month?.cancelledOrders },
                  {
                    label: "Doanh thu",
                    value:
                      Number(month?.revenue || 0).toLocaleString("vi-VN") + "đ",
                  },
                ].map((s, i) => (
                  <div
                    key={i}
              ...
              <div className="space-y-2">
                {month?.topItems?.slice(0, 6).map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <span className="flex-1 text-sm text-coffee-700 truncate">
                      {item.tenMon}
                    </span>
                    <div className="flex items-center gap-2">
                      <div
                        className="h-2 bg-coffee-400 rounded-full"
                        style={{
                          width: `${Math.min(100, (item._sum?.soLuong / (month.topItems[0]?._sum?.soLuong || 1)) * 80)}px`,
                        }}
                      />
                      <span className="text-xs font-bold text-coffee-700 w-8 text-right">
                        {item._sum?.soLuong}
                      </span>
                    </div>
                  </div>
                ...
                {(!month?.topItems || month.topItems.length === 0) && (
                  <p className="text-coffee-400 text-sm text-center py-4">
                    Chưa có dữ liệu
                  </p>
                )}
              </div>
